...
function AnalyticsItem({metric, icon}: AnalyticsProps) {
  return (
    <div className="text-white flex items-center flex-row justify-center px-[31px] py-[3.5px]">
      ...
      <span className="text-[12.05px] font-semibold font-poppins">
        { metric || 0 }
      </span>
    </div>
  )
...
export default function Card({ shop }: { shop: Shop }) {
  ...
  return (
    <motion.div
      ...
      className="relative group cursor-pointer bg-light px-4 py-7 pb-[90.17%] author-tile"
    >
      <div className='absolute inset-0 h-full w-full flex flex-col p-[23.78px] pt-[94.7px]'>
        <div className="flex">
          ...
          <div className="flex items-center">
            <div>
              <h3 className="text-[24.11px] text-[#fefefe] font-semibold font-poppins  transition-colors group-hover:text-brand duration-200">
                {name}
              </h3>
              <p className="text-[13.78px] text-[#bbb] font-medium font-poppins italic">
                @{slug}
              </p>
            </div>
          </div>
        ...
        <div className="mt-[20.66px] flex justify-center divide-x divide-[#D9D9D9]">
          <AnalyticsItem metric={users_count} icon={<UserHeadShouldersIcon className="w-[13.78px] h-[18.08px]" />} />
          <AnalyticsItem metric={wishlists_count} icon={<HeartFillIcon className="h-[15.91px] w-[17.22px]" />} />
          <AnalyticsItem metric={orders_count} icon={<DownloadCloudIcon className="h-[17.58px] w-[18.94px]" />} />
        </div>
