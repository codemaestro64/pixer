...
import ServiceCardLoader from '@/components/service/card-loader';
...
export default function Grid({
  ...
  if (!isLoading && !gigs.length) {
    return (
      <ItemNotFound
        title={t('text-no-gigs-found')}
        message={t('text-no-gigs-found-message')}
    ...
    <div className="px-4 pt-5 pb-9 md:px-6 md:pb-10 md:pt-6 lg:px-7 lg:pb-12 3xl:px-8">
      ...
      >
        {isLoading && !gigs.length
          ? rangeMap(limit, (i) => (
              <ServiceCardLoader key={i} uniqueKey={`post-${i}`} />
            ))
          : gigs.map((gig) => <Card key={gig.id} gig={gig} />)}
      </motion.div>
