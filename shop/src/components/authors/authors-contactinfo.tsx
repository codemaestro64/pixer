...
function Contact({ text, icon, phone }: { text?: string, icon?: React.ReactElement, phone?: any }) {
  return (
...
export default function AuthorsContactInfo({ email, address }: any) {
  return (
    <div className='py-[20px] px-[23px] bg-white dark:bg-[#292929] rounded-[20px]'>
      ...
      <div className='mt-[13.71px] space-y-[13.71px]'>
        <Contact phone={{ code: '+12', number: '222-323-9898'}} icon={<PhoneFillIcon />} />
        <Contact text={email} icon={<MailOpenFillicon />} />
        <Contact text={address.street_address} icon={<MapPinFillIcon />} />
      </div>
    </div>
  )
}